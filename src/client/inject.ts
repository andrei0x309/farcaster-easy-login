...
function createFCMessage (data: any) {
...
const generateApiToken = async (): Promise<T_RESULT_GEN_AUTH_TOKEN> => {
    try {
        ...
        const req = await fetch(`${FARCASTER_XYZ_API}/auth`, {
            method: "PUT",
            headers: {
...
const addFCAuthToken = async (value: T_IDDB_VALUE):  Promise<unknown> => {
...
window.addEventListener('message', async (event) => {
    ...
    if (event.data.type && event.data.type === 'farcaster-login') {
